...
- [x] Put the official dataset in `data/raw/banking.csv` and `data/raw/banking_test.csv`.
- [x] Run `python scripts/train.py --train data/raw/banking.csv --target churn`.
- [x] Run `python scripts/predict.py --model models/churn_model.joblib --test data/raw/banking_test.csv --out outputs/predictions.csv`.
